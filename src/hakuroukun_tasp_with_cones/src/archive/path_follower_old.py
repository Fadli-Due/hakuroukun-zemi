...
class PurePursuitNode:
    ...
    def compute_pure_pursuit(self):
        ...
        rospy.loginfo(f"[PP] Alpha: {math.degrees(alpha):.2f} deg | dx: {dx:.2f}, dy: {dy:.2f}")
        rospy.loginfo(f"[PP] Robot yaw: {math.degrees(yaw):.2f} deg | Lookahead: ({Ld_x:.2f}, {Ld_y:.2f})")
        ...
        # -----  OPTIONAL GUARD  -----
        # Don’t allow reverse unless the look‑ahead point is truly behind us (>100°)
        if desired_speed < 0 and abs(alpha) < math.radians(100):
            desired_speed = 0.0
        # ----------------------------
        ...
        rospy.loginfo(f"[PP] New Speed: {new_speed:.2f} | Steering: {steering:.2f}")
